...
class TaskGenerator():
    ...
    def task_sampling_default_dis(self, episode_task_num=10000):
        self.step_counter += 1
    ...
    def task_sampling_default_rand(self, episode_task_num=10000):
        self.step_counter += 1
        if self.step_counter >= len(self.time_list):
            self.step_counter = 0
        key_t = self.time_list[self.step_counter] # 时间段
    ...
    # episode fix
    def task_sampling_fix(self, inner_step, episode_task_num=10000):
        time_index = inner_step % len(self.time_list)
        key_t = self.time_list[time_index] # 时间段
        task_samples_num = episode_task_num
        rest_sample_num = task_samples_num - len(self.task_candidates[key_t])
        if rest_sample_num <= 0:
            task_samples = self.task_candidates[key_t][:task_samples_num]
        else:
            while rest_sample_num > 0:
                task_samples += self.task_candidates[key_t]
                if rest_sample_num <= len(self.task_candidates[key_t]):
                    task_samples += self.task_candidates[key_t][:rest_sample_num]
                    break
                rest_sample_num = rest_sample_num - len(self.task_candidates[key_t])
        return task_samples

    # totally fix
    def task_sampling_fix_step(self, episode_task_num=10000):
        key_t = self.time_list[0] # 时间段
        task_samples_num = episode_task_num
        rest_sample_num = task_samples_num - len(self.task_candidates[key_t])
        if rest_sample_num <= 0:
            task_samples = self.task_candidates[key_t][:task_samples_num]
    ...
    # 按照时间段
    def task_sampling_fix_t(self, episode_task_num=10000):
        self.step_counter += 1
        if self.step_counter >= len(self.time_list):
            self.step_counter = 0
        key_t = self.time_list[self.step_counter] # 时间段
    ...
    # fix
    def task_sampling_poisson_fix(self, step_num, episode_task_num, max_task_size=100):
        ...
        if task_samples_num == 0:
            task_samples_num = 1
        task_samples = self.task_sampling_fix(task_samples_num)
        return task_samples
