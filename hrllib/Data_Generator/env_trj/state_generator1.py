    ...
    def reward_compute(self):
        ...
        for task_id in self.finished_schedules:
            ...
            if task[9] == 0: # not count yet
                ...
                self.reward += 1.0 * 0.05 # finish reward
                #print "(p%s_t%s_finish)+%s" % (pid, task_id, "0.05"), 
                if pid in self.participant_finish_task:
                    self.participant_finish_task[pid] += 1
                else:
                    self.participant_finish_task[pid] = 1

                self.reward += fare_amount * 5.0  #fare*5
                #print "(p%s_fare)+%s" % (pid, fare_amount * 5.0), 
