#On-Policy Learning and TD(0) Learining
#SARSA
import random
import time
        ...
        for epoch in range(epoch_num):
            ...
            for i in samples:
                if self.replay_done[i]:
                    ...
                    _, Q_obs, Q_debug = policy.action_sel([self.replay_next[i]])
                    #print Q_obs, self.replay_rew[i] + self.gamma * Q
                    #different with DQN
                    samples_Q.append(self.replay_rew[i] + self.gamma * Q_debug[0, self.replay_act[i+1] - 1])
            #print samples_Q
