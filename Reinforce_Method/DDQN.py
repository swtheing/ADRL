...
class DDQN(Reinforce_Suite):
    def __init__(self, config, game_name,  env):
        if config.model == "DNN":
            model_reserve = Perceptron(game_name, None, config, "MSE")
            model_predict = Perceptron(game_name, None, config, "MSE", "predict", model_reserve)
        else:
            model_reserve = Cnn(game_name, None, config, "MSE")
            model_predict = Cnn(game_name, None, config, "MSE", "predict", model_reserve)
        Reinforce_Suite.__init__(self, config, model_reserve, env, model_predict)
