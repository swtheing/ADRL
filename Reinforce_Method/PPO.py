...
class PPO(Reinforce_Suite):
    def __init__(self, config, game_name,  env):
        if config.model == "DNN":
            ...
            Actor = Trans_Ptr(game_name, None, config, "PPO_L", "Actor", new_graph = True)
            self.Old_Actor = Trans_Ptr(game_name, None, config, "PPO_L", "Old_Actor", copy_model = Actor)
            self.conti_act = False
            self.multi_act = True
    ...
    def Gen_Batch_Data(self, policy, epoch_num):
        ...
        for epoch in range(epoch_num):
            ...
            old_prob, _ = self.Old_Actor.test_model(self.replay_obs)
            for i in samples:
                ##Todo
                task_ratio = []
                for j in range(len(old_prob[i])):
                    task_ratio.append(old_prob[i][j][self.replay_act[i][j]-1])
                samples_ratio.append(task_ratio)
            ...
            print "samples_ratio:"
            print samples_ratio
            tup = (samples_obs, samples_act, samples_epr, samples_val, samples_next, samples_ratio)
    ...
    def Train_Data(self, policy, train_epoch, train_data, rescale = True):
        ...
        #print self.replay_Q
        samples_obs, samples_act, samples_epr, samples_val, samples_next, samples_ratio = train_data
        #self.base_v = samples_val
        mean_reward = np.mean(samples_epr)
        self.base_v = self.base_v * 0.9 + mean_reward * 0.1
        print "base_v:"
        print self.base_v
        if rescale: 
            ...
            print samples_act
        self.Old_Actor.copy()
        policy.model.train_model(samples_obs, samples_act, samples_epr, origin_epr, samples_next, train_epoch, samples_ratio)
